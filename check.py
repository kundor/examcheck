...
def get_args(argv=sys.argv):
    ...
    if argv[-1].endswith('.xlsx'):
        ...
        lastsub = len(argv)
        origfiles = list((basedir() / 'original').glob(f'Module-{modnum}-*.xlsx'))
...
def reportinfo(info, self_created_ok=False):
    ...
    if not isinstance(info.modified, datetime):
        pass
    elif info.modified > examtime + timedelta(days=1) or info.modified < examtime - timedelta(days=9) and stat is not Status.Boo:
        if not domodmsg:
